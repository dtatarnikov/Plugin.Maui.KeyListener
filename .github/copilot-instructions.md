...
- **iOS/Mac** (`KeyboardBehavior.iOS.cs` + `KeyboardPageViewController.cs`): Replaces `PageHandler.PlatformViewFactory` with a custom `KeyboardPageViewController` that overrides `PressesBegan`/`PressesEnded`/`PressesCancelled`. Uses weak references to track registered behaviors.
- **Windows** (`KeyboardBehavior.Windows.cs`): Hooks `XamlRoot.Content` KeyDown/KeyUp events on the root `FrameworkElement`
...
- **Tabs for indentation** (4-wide) in C# files, as defined in `.editorconfig`
